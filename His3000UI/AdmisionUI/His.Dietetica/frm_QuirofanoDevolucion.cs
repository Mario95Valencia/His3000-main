...
{
    public partial class frm_QuirofanoDevolucion : Form
    {
        public frm_QuirofanoDevolucion()
        {
            InitializeComponent();
        }
        public frm_QuirofanoDevolucion(int bodega)
        {
            InitializeComponent();
            this.bodega = bodega;
        }
        ...
        public int bodega = Convert.ToInt32(NegParametros.ParametroBodegaQuirofano());
        private void frm_QuirofanoDevolucion_Load(object sender, EventArgs e)
        {
            ...
            if (ultraGridPedido.Selected.Rows.Count == 1)
            {
                ...
                if (!NegParametros.ParametroDevolucionBienes())
                {
                    ...
                    if ((Convert.ToDecimal(Fila.Cells[4].Value) - Convert.ToDecimal(Fila.Cells[5].Value)) - (Convert.ToDecimal(txtCantidad.Text)) >= 0) // Verifica si la existe la cantidad seleccionada / Giovanny Tapia / 04/10/2012
                    {
                        ...
                        PedidosDetalleItem.DevDetIva = ((((Convert.ToDecimal(Fila.Cells[6].Value) * Convert.ToInt32(txtCantidad.Text))) * Convert.ToDecimal(Fila.Cells[7].Value)) / 100);
                        PedidosDetalleItem.DevDetIvaTotal = Convert.ToDecimal(Fila.Cells[6].Value) * Convert.ToInt32(txtCantidad.Text);
        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            ...
            if (ultraGridDevolucion.Rows.Count > 0)
            {
                if (txtObservacion.Text.Trim() != "")
                {
                    ...
                    Int64 DevolucionNumero = NegPedidos.CrearDevolucionQuirofano(Devolucion, Convert.ToInt64(ate_codigo), 0, bodega, modulo); //DENTRO EDITA LA REPOSICION 

                    if (DevolucionNumero != 0)
                    {
                        MessageBox.Show("La devolucion No." + " " + DevolucionNumero.ToString() + " a sido ingresada correctamente.", "His3000", MessageBoxButtons.OK, MessageBoxIcon.Information);
