...
{
    public partial class frm_AccesosCg : Form
    {
        Int64 id_modulo;
        Int64 id_usu;
        public frm_AccesosCg()
        {
            InitializeComponent();
            CargaUsuarios();
        }
        ...
        {
            ultraGridUsuario.DataSource = NegUsuarios.UsuariosCG();
        }
            ...
            foreach (UltraGridRow item in ultraGridModulo.Rows)
            {
                if (item.Cells["ID"].Value.ToString() == e.Cell.Row.Cells["ID"].Value.ToString())
                {
                    try
                    {
                        ...
                        ultraGridAccesos.DataSource = NegUsuarios.BuscaPerfilesCg(Convert.ToInt64(e.Cell.Row.Cells["ID"].Value.ToString()), id_usu);
        ...
        private void ultraGridUsuario_DoubleClickCell(object sender, Infragistics.Win.UltraWinGrid.DoubleClickCellEventArgs e)
        {
            foreach (UltraGridRow item in ultraGridUsuario.Rows)
            {
                if (item.Cells["ID"].Value.ToString() == e.Cell.Row.Cells["ID"].Value.ToString())
                {
                    ...
                    {
                        ultraGridAccesos.DataSource = null;
                        ultraGridModulo.DataSource = NegUsuarios.ModuloCG();
                    ...
                    foreach (var item in ultraGridAccesos.Rows)
                    {
                        if (item.Cells["ID"].Value.ToString() == e.Cell.Row.Cells["ID"].Value.ToString())
                        {
                            if (!Convert.ToBoolean(item.Cells["TIENE_ACCESO"].Value.ToString()))
                            {
                                ...
                                if (!NegUsuarios.CrearPerfilCg(id_usu, Convert.ToInt64(e.Cell.Row.Cells["ID"].Value.ToString()), "S", " "))
                                {
                                    MessageBox.Show("No se ha podido crear el acceso ", "Cg-3000", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                }
                                ultraGridAccesos.DataSource = NegUsuarios.BuscaPerfilesCg(id_modulo, id_usu);
                            }
                            else
                            {
                                if (!NegUsuarios.CrearPerfilCg(id_usu, Convert.ToInt64(e.Cell.Row.Cells["ID"].Value.ToString()), "N", " "))
                                {
                                    MessageBox.Show("No se ha podido crear el acceso ", "Cg-3000", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                }
                                ultraGridAccesos.DataSource = NegUsuarios.BuscaPerfilesCg(id_modulo, id_usu);
