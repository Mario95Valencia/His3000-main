...
namespace His.Admision
{
    public partial class frm_Ayudas : Form
    {
        ...
        public bool medi = false; //para que no afecte la ayuda con respecto a paises
        #region Constructor
        public frm_Ayudas()
        {
            InitializeComponent();
        ...
        public frm_Ayudas(List<PAIS> nacionalidades, string tablaExt, string columnBusq)
        {
            ...
            grid.ActiveCell = grid.Rows.FilterRow.Cells[0];
            grid.PerformAction(UltraGridAction.EnterEditMode);
            //grid.Columns["NACIONALIDAD"].Width = 300;
        }
        public frm_Ayudas(List<MEDICOS> medicos, string tablaExt, string columnBusq,string Tipo)
        {
            medi = true;
            try
            {
            ...
            if (_Tipo == "EMERGENCIA")
            {
                grid.DataSource = medicos/*.OrderBy(p => Convert.ToInt32(p.MED_TELEFONO_CONSULTORIO))*/.Select(n => new
                {
                    CODIGO = n.MED_CODIGO,
        ...
        private void enviarCodigo()
        {
            if (grid.ActiveRow.Index > -1)
            {
                if (campoPadre != null)
                {
                    ...
                    if (medi == true){
                        especialida = grid.ActiveRow.Cells["ESPECIALIDAD"].Value.ToString();
                        nombremedico = grid.ActiveRow.Cells["NOMBRE"].Value.ToString();
                    }
